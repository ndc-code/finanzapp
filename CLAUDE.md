...
**Never trust a Realtime payload as a whole row.** `payload.new` can arrive empty (`{}` with an `errors` entry) when the row exceeds Realtime's max payload size — likely for a space with months of history — or missing columns when pushes reorder. `isWholeCoupleRow()` gates this: an incomplete push triggers a `fetchCoupleRow()` refetch instead of being adopted. `applyCoupleRow()` then overwrites each in-memory field **only when the row actually carries it** (`if(Array.isArray(coupleRow.expenses)) expenses = …`) — a missing column means "unchanged", never "empty". The earlier code reset `expenses`/`income` to `[]` on any incomplete payload, which silently wiped whole months of data. `saveExpenses()`/`saveIncome()` also refuse a write that takes a non-empty set straight to empty (`bailOnMassWipe()`) and resync from the row instead — the UI only ever deletes one entry at a time.

**Backups** live in a separate `couple_backups` table (`code`, `created_at`, `label`, `snapshot jsonb`), *not* a column on `couples` — a snapshot column would bloat the row and worsen the Realtime-truncation hazard above. Schema + RLS in `docs/couple_backups.sql` (run once in Supabase; `42P01` from the app means it hasn't been run). The backup module (search `BACKUP_TABLE`) does on-demand `select`/`insert`/`delete` when Ajustes opens — no Realtime. `currentSnapshot()` captures every data column; `applySnapshot()` restores them in one atomic `update` to `couples`, and every restore path auto-creates an "Antes de restaurar" backup first. Ajustes also exposes `.json` download/upload as a Supabase-independent fallback.
